...
import { ShieldAlert, CheckCircle2, Clock, Truck, Lock, MessageSquare, QrCode, LogOut, Loader2, Check, RotateCw, Sparkles, Smartphone } from 'lucide-react';
          ...
          {waStatus === 'DISCONNECTED' && (
            <div className="flex flex-col items-center text-center py-4">
              {waQr ? (
                ...
                <div className="w-48 h-48 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center justify-center gap-2 mb-4">
                  <Loader2 className="w-8 h-8 text-blue-500 animate-spin" />
                  <span className="text-xs text-gray-500 font-mono">Gerando QR Code...</span>
                </div>
              )}
              
              <h4 className="text-white text-xs font-bold uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                <Smartphone className="w-4 h-4 text-emerald-400" /> Pareamento com Celular
              </h4>
              <p className="text-xs text-gray-400 leading-relaxed max-w-[280px] mb-4">
                Escaneie o código acima com o WhatsApp do seu celular para vincular a central de alertas da Defesa Civil.
              </p>

              <div className="w-full max-w-xs space-y-2">
                <button
                  onClick={handleSimulateConnect}
                  className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-mono font-bold text-[11px] uppercase tracking-wider shadow-lg shadow-emerald-600/20 cursor-pointer active:scale-95 transition-all flex items-center justify-center gap-2"
                >
                  <Sparkles className="w-3.5 h-3.5 text-emerald-200" />
                  Simular Pareamento Imediato (Demo)
                </button>

                <button
                  onClick={fetchWaStatus}
                  className="w-full py-2 px-3 rounded-lg bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white font-mono text-[10px] uppercase tracking-wider border border-white/5 transition-all cursor-pointer flex items-center justify-center gap-1.5"
                >
              ...
              <div className="mt-4 pt-3 border-t border-white/5 text-[9px] font-mono text-gray-500 max-w-[300px]">
                ℹ️ Suporta worker WhatsApp Web local ou canal de alerta comunitário em nuvem.
              </div>
            </div>
          )}
